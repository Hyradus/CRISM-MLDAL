...
def make_folder(PATH):
    savepath = PATH+'/multi_DecTree-RanFor_models'
...
def DecTree_train(X_train, Y_train):
    print('\nTraining Decisional Trees Regressor')
    #Model training
    from sklearn.tree import DecisionTreeRegressor
    dTree = DecisionTreeRegressor(max_depth=15, random_state=0)
    start = timer()
    dTree_model = dTree.fit(X_train, Y_train)
    end = timer()
    print('\nTrained in: ', end-start, 's')
    return(dTree_model)

def DecTree(X_train, X_test, Y_train, Y_test, Xs, Ys):
    dTree_model = DecTree_train(X_train, Y_train)
    model_name = 'DecTreeReg'
    save_models(dTree_model, model_name, savepath)
    results, scores = evaluate(dTree_model, model_name, Xs, Ys, valjobs=8)
...
def RanFor_train(X_train, Y_train):
    print('Training Random Forest Regressor')
    #Model training
    from sklearn.ensemble import RandomForestRegressor
    RanFor = RandomForestRegressor(n_estimators=2000, max_depth=20, random_state=1, n_jobs=JOBS)
    start = timer()
...
def RanForest(X_train, X_test, Y_train, Y_test, Xs, Ys):
    RanFor_model = RanFor_train(X_train, Y_train)
    model_name = 'RanForestReg'
    save_models(RanFor_model, model_name, savepath)
    results, scores  = evaluate(RanFor_model, model_name, Xs, Ys, valjobs=1)
...
def main(X_train, X_test, Y_train, Y_test, Xs, Ys):    
    ...
    # DecTree(X_train, X_test, Y_train, Y_test, Xs, Ys)
    RanForest(X_train, X_test, Y_train, Y_test, Xs, Ys)
